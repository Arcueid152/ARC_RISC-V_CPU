...
module EX (
    ...
    //新增
    input  wire        pred_taken_in,  // 当初IF阶段的预测的是否跳
    output reg         is_branch,      // 告知BPU：这是一条分支指令，需要更新PHT/BTB
    output reg         mispredict      // 告知IF/IF2ID/ID2EX：预测失败，需要冲刷
);
    ...
    case (opcode)
      ...
      `INST_TYPE_B: begin
        ...
        begin
          ...
          begin
          // 实际没跳，但预测跳了 → 预测失败，需要冲刷
          if (pred_taken_in) 
            begin
            mispredict = 1'b1;
            jump_hold  = 1'b1;   // 复用 jump_hold 来冲刷 IF2ID 和 ID2EX
            jump_addr  = instr_addr_in + 32'h4;  // 修正PC为顺序下一条
            end
          end
      end
